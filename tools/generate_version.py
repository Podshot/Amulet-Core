...
def generate_version_from_dict(dict_data: dict) -> Tuple[Tuple[int, int, int], int, str]:
    return dict_data["version_number"], dict_data["version_int"], dict_data["version_stage"]
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        description="Generate a version.json file for an Amulet release"
    )
    parser.add_argument(
        "-i",
        "--int",
    ...
    if os.path.exists(args.input):
        with open(args.input) as fp:
            ...
            args.version, args.int, args.stage = generate_version_from_dict(version_data)
